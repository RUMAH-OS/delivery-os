...
| **config-doctor + config-registry + pre-build config-gate** (Infrastructure Config Platform) | the PLOS prod-deploy saga: 4 failure LAYERS discovered one-failed-build-at-a-time (Node-24 → `apps/web` path-doubling #197 → empty-optional `SUPABASE_URL` crashing zod `.optional().url()` #200 → genuinely-missing required `DATABASE_URL`) → multi-hour one-by-one secret discovery | gate+automation | OS-foundational | **verified** (self-test 16/16; FIELD-PROVEN: rumah-admin gate failed-closed on PR #17 listing ALL 4 missing keys at once — the inversion of the saga). NOT yet on the deploy branch — BUG-4 (REQUIRE-MERGE before cutover) | `templates/tools/config-doctor.mjs` + `config-registry.json` + `templates/workflows/config-gate.yml` (PRs delivery-os #15 / PLOS #201 / Admin #17) |
| **platform-health / rollback-helper / post-deploy-verify / diagnose** (self-healing layer) | a deploy could fail or degrade with no health taxonomy, no rollback helper, no post-deploy verification | gate+automation | OS-foundational | **verified** (self-tests green both repos: platform-health 30/30 · rollback-helper 7/7 · post-deploy-verify 10/10; integration covered) — proven in e2e `docs/verify/E2E-WORKFLOW-VERIFICATION-2026-06-27.md` §5 | `infra/platform-health.mjs` + rollback-helper + post-deploy-verify (#16/#202/#18) |

## Open backlog (candidates — feed the chain forward)
| Item | Founder reality | Kind | Priority | Why |
|---|---|---|---|---|
| **deploy-preflight** (one pass: config-doctor + build-shape + migration-state → ONE report of all blockers) | the PLOS deploy saga MASKED failures — each fix unmasked the next layer (Node-24 → path-doubling → empty-optional zod → missing-required), so the founder hit N deploy cycles instead of one | automation | **HIGH** | collapses the masking saga: surfaces the COMPLETE blocker set up front (the config-gate already lists all missing config at once; extend to build-shape + migration-state). Field-earned 2026-06-27; Learning Review L5 |
| **infrastructure-registry** (`ecosystem-architecture/12-infrastructure-registry.md`) | pooler INVERSION (Admin session-`:5432` vs PLOS transaction-`:6543`, both correct per workload), team-vs-personal Vercel scope drift, Supabase refs + Vercel ids recorded nowhere — all silent until inventoried | gate-able registry | **HIGH** | records reality (incl. deliberate divergence) so it can't drift again: per-app Supabase ref · Vercel id+scope · pooler mode + rationale · Node ver · cron+plan · deploy mechanism · env-key NAMES. Seeded by `docs/audits/INFRASTRUCTURE-INVENTORY-2026-06-27.md`; amend ECR-0005 for Admin's session-pooler exception. Ecosystem layer (DESIGN-FIRST, not written from this retro) |
| **env-schema blank→undefined coercion** (the empty-optional trap) | a present-but-EMPTY env var (`SUPABASE_URL=""`) crashed zod `.optional().url()` at boot (#200) — `.optional()` sees `""` as present, `.url()` rejects it | doctrine+template | **MED** | the recurring env-schema pattern every future capability must inherit: `loadEnv()` coerces blank→`undefined` before validation + config-doctor validates declared optionals so an empty-optional fails the gate, not the boot. Built (the #200 fix); record as the standard |
| **runbook/remediation reference-integrity** (extend check-hook-paths to operator-facing text) | BUG-4: `main` advertised `config-doctor.mjs` in remediation strings + runbook while the file lived only on an unmerged branch — the prior hook-path gate checks skill tool refs, not runbook prose | gate | MED | a runbook must not promise a tool absent from the branch that ships it; earned by BUG-4 (the capability that should have caught it). Learning Review Q4 |
...
## Preserve (anti-regression — strengths the milestone proved, do NOT "fix")
- **2026-06-27 (Infrastructure Platform):** the **verify-gate + auto-mode classifier correctly gated
  founder-only merges/secrets** throughout the milestone — no secret leaked, no self-merge, every REQUIRE-PROD
  leg stayed founder-gated, and **independent QA (author≠verifier) caught BUG-1..4 BEFORE the founder hit them
  in prod** (the discovery rate trending to zero). This is the floor working as designed. Recorded as a
  strength to PRESERVE — explicitly NOT a capability to change. (Learning Review L4 + Q4 anti-decay win.)
