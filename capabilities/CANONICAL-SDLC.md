...
Smoke Tests → Production Verify → Release Notes → Branch/PR Cleanup → Learning Review
```
...
| Local Verify / Commit | author≠verifier gate; a VERIFY's freshness is **SEMANTIC (impl-fingerprint), not timestamp** — valid until the verified impl's runtime behavior changes (`DECISIONS.md` D8). **Enforcement is server-side (D9):** the local hook is a non-blocking **ADVISORY + auto-verify** nudge; the BINDING gate is the required CI `verify-coverage` check + branch protection + CODEOWNERS at the PR/merge | REUSE | `.claude/hooks/verify-gate.mjs` (advisory), `skills/verify-gate`, `templates/tools/verify-fingerprint.mjs` |
| Verify Coverage (BINDING) | the irreversible, model-independent verification gate (D9): required CI status check re-runs `machine_probe` on neutral hardware + confirms a fresh independent VERIFY semantically covers the changed impl; branch protection makes author≠verifier a platform invariant; CODEOWNERS binds a second principal at the merge | **NEW** (CI/branch-protection owned elsewhere) | `.github/workflows/verify-coverage.yml`, `setup-branch-protection.mjs`, `verify-fingerprint.mjs` |
...
| Verification | verification is a system behavior; "fresh VERIFY" = **semantically fresh** (impl-fingerprint match), not newer-than-mtime — a behavior change still demands a fresh independent VERIFY (D8). **The binding enforcement is the CI `verify-coverage` required check + branch protection + CODEOWNERS at the PR/merge (D9), not the local hook** (advisory) — verification is enforced where it is un-bypassable and model-independent | REUSE | CI `verify-coverage` + branch protection + CODEOWNERS · `verify-gate` (advisory) + `qa-test` agent |
| Learning Review | a registered first-class lifecycle hook (peer to the Verify + Founder-Review rows) — **trigger = `learning-trigger.mjs` levels** (L0 continuous capture / L1 lightweight checkpoint / L2 full multi-specialist review on the heavy triggers: incident · ADR/board · new-capability · founder-epic · milestone · census-candidate · N-merge backstop>30); `learning_expected = L1∨L2`, default no-review, **fail-open** (L0/L1 never block); every approved lesson auto-classified (`learning-classify.mjs`, the 10-way taxonomy) + inherited; an L2 emits the **Founder Learning Package**; completeness via the §14 N-merge backstop. Feeds the §14 promotion machinery — promotion bar + §11 economics UNCHANGED (`DECISIONS.md` D10, `docs/adr/ADR-003`) | REUSE+**NEW-thin** | `learning-trigger.mjs` + `learning-classify.mjs` + `founder-learning-package.mjs` (net-new) over `file-lesson.mjs`, `signals.jsonl`, `census-detector.mjs`, `agent-health.mjs`, `learning-review` skill (reuse) |
...
- **Phase 5 — Close the learning loop (REGISTERED, `DECISIONS.md` D10 / `docs/adr/ADR-003`):** **Learning Review** is now a first-class lifecycle hook, not an unbuilt terminal step — the 3-level `learning-trigger.mjs` policy + `learning-classify.mjs` (the 10-way taxonomy) + the `founder-learning-package.mjs` L2 artifact, composing `learning-review` + `milestone-report` + S2 agent-health persist. SHADOW rollout (classify + log before gating).
