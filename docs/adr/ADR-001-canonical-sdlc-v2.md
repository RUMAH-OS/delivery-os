...
- **Safety floor unchanged.** `founder_verifiable` is **orthogonal to Class C**: Class C consequential decisions (money/payments/pricing/auth/contracts/e-signatures/PII/migrations/prompts/secrets/control-plane) and irreversible actions (prod deploy, merge-to-main) remain human-gated (Governance §6/§11/§16) regardless of founder-verifiability — "real founder decisions that cannot be automated." This refinement narrows *validation* interruptions; it does not relax any safety gate, and the D3 auto-merge stays DEFERRED.
