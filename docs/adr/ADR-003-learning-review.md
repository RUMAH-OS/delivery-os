...
- **Status:** **ACCEPTED — 2026-06-25 (founder-ratified).** Registers Learning Review as a first-class Delivery OS lifecycle hook (peer to VERIFY and Founder Review) governed by a ratified 3-level trigger policy. The board scored the alternatives and converged; the founder ratified the capability. It runs in **SHADOW** (classify + log, no blocking review fired) — **turning on gating is a separate founder decision** once false-fire/missed-fire rates are observed on real work. The 8 learnings captured in the ratifying review were held (filed as signals for a later census/promotion decision), not installed. Relates to `DECISIONS.md` **D10**.
- **Date:** 2026-06-25
