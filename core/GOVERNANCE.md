...
- **The BINDING gate is the required CI `verify-coverage` check + branch protection + CODEOWNERS at the PR/merge (board ratified 2026-06-25, founder-approved — `DECISIONS.md` D9).** Enforcement lives where it is **model-independent and un-bypassable**: a REQUIRED GitHub status check (`verify-coverage`, `.github/workflows/verify-coverage.yml`) re-runs the slice's `machine_probe` on neutral hardware and confirms a **fresh, independent `VERIFY-<slice>.md` semantically covers the changed impl** (the normalized `impl_fingerprint`, near-zero-false-positive); **branch protection** (provisioned by `setup-branch-protection.mjs`) makes **author≠verifier a platform invariant** — GitHub itself bars a self-approval — and **CODEOWNERS review** binds a second principal at the irreversible act (the merge). This is the layer the old §12 Honest Limit pointed to: it is *fully model-independent*, re-runs the probe off the author's machine, and cannot be bypassed by `--no-verify`, a bypass env var, an unconfigured clone, or a web/non-Claude merge.
- **The local verify-gate hook is DEMOTED to a non-blocking ADVISORY + auto-verifier (it NEVER blocks).** A `.claude/settings.json` hook (`PostToolUse` baseline + advisory warn; an auto-verify nudge that *offers* to produce the missing `docs/verify/VERIFY-<slice>.md`) surfaces a coverage gap early and cheaply — but the irreversible decision is made server-side, not at the reversible local commit. The hook gated a **reversible** commit while the **irreversible** merge ran under a weaker check, carried 4+ bypass paths, and could not prove independence (§12 Honest Limit) — and its false positives (dozens per goal, >60% on non-functional churn) bred habituated bypass, a *safety regression*. As a fast, honest nudge it raises the cost of forgetting; it no longer pretends to be the binding control (§13: mechanism belongs where it's model-independent).
- **A VERIFY is invalidated SEMANTICALLY, not by timestamp (founder directive 2026-06-25).** The gate no longer treats a VERIFY as stale merely because the impl's mtime is newer. A VERIFY records a **normalized `impl_fingerprint`** of the code it verified and **remains valid until that code's runtime behavior changes** (a fingerprint mismatch, computed via `templates/tools/verify-fingerprint.mjs`). **Non-functional changes never invalidate** it — docs, governance, skills, capabilities, templates, registry, rebases, merge commits, formatting, generated files, metadata all keep the fingerprint and keep the VERIFY valid. **Same safety floor:** a real behavior change still produces a mismatch and still requires a **fresh, independent** VERIFY before the slice can advance — the gate has only stopped punishing cosmetic churn, not relaxed the author≠verifier requirement. A legacy VERIFY with no `impl_fingerprint` field falls back to the old mtime staleness check.
...
**Honest limit (stated — and now RESOLVED for git+CI repos, board 2026-06-25).** A *local hook* can enforce that a fresh, passing, independence-*claimed* artifact **exists**; it cannot prove the verification was *truthful* or genuinely independent inside a single-agent runtime. That limit is **why the binding gate moved server-side**: only **CODEOWNERS-on-a-real-PR-with-a-second-reviewer** can prove independence, so for any repo with git + CI the required `verify-coverage` check + branch protection + CODEOWNERS **are** the model-independent layer the limit pointed to — the platform, not a remembered choice, enforces author≠verifier and re-runs the probe on neutral hardware. The residual limit survives only in the **no-VCS fallback** (a single principal, no PR to bind), where a *separate verifier run* is the strongest available form. This is why git (§12 link 1) is mandatory, not advisory — without it there is no platform to host the binding gate.
...
- **KERNEL MECHANISM** (thin · non-swappable · fires without consent · fails closed): the **required CI `verify-coverage` check + branch protection + CODEOWNERS** that bind verification at the irreversible PR/merge, the git substrate (no-git ⇒ no-build), the derived `verify_status` engine, the drift-lint, key/capability-scoping, and **author≠verifier as a structural write-binding** (the verifier's write-scope excludes the code it grades — CODEOWNERS where git exists; a distinct verifier run where it does not). These are installed by the scaffolder and run whether or not anyone chooses to invoke them. The **local verify-gate hook is NOT in this set** (board 2026-06-25): it is the honest *advisory* it can be — a fast local nudge + auto-verifier that never blocks — because a local hook gating a reversible commit could be bypassed and could not prove independence (§12).

**Canonical example of the line (the verification gate, board-ratified 2026-06-25 — `DECISIONS.md` D9, `docs/adr/ADR-002`).** Verification enforcement is the textbook case of "mechanism belongs where it's model-independent." The **binding mechanism lives server-side** — GitHub **branch protection** makes author≠verifier a *platform invariant* (the platform bars self-approval), the **required `verify-coverage` status check** re-runs the `machine_probe` on neutral hardware off the author's box, and **CODEOWNERS** binds a second principal at the merge. The **local hook stays the honest advisory it can be** — it fires without consent and is useful, but it gated a *reversible* commit, carried bypass paths, and could not prove the verification was independent; per the *"if it can, it will"* test, that made it a hope dressed as a key. Moving the binding layer to where the irreversible act happens (the merge, server-side) is the §13 doctrine applied to the OS's own flagship control.
...
**The verify-gate Stop-block demotion closes the matching local incident (board 2026-06-25 — §12/§13, `DECISIONS.md` D9).**
The local hook's `Stop`-block manufactured a **pre-boundary boundary**: it halted a *reversible* commit/turn at a point that was not a real founder boundary, contradicting this section's rule that the only true boundaries are the human-gated tier (C6 · merge · prod · rollback). Demoting the hook to a non-blocking advisory + auto-verifier ends that class of idle loop at the source — the hook no longer manufactures a boundary the goal can never autonomously clear; the binding verification gate now fires server-side at the *actual* irreversible act (the PR/merge), exactly where §16 says a boundary belongs.
