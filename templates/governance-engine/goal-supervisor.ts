...
// =============================================================================
// PLATFORM EXTRACTION SLICE 2 — the FIRST organ inverted onto the ports. This is the port-injected mirror of
// `rumah-admin/src/goal-supervisor-c7.ts`. The admin module imports the durable-store functions directly from
// `./runtime-stores.js` (which imports `./db/client.js`); THIS module imports NO DB — every durable read/append
// crosses an injected `RuntimeStoresPort` (and the GS composes a `GoalContractStorePort` to source the goal's
// budget envelope + lifecycle state). The residency invariant is enforced by `residency-guard.mjs`.
//
// WHAT CHANGED vs admin (and ONLY this): the STORE-ACCESS PATH. The four durable reads/appends the GS performs
// (appendProgressSample / readProgressSeries / countAttempts / readCumulativeCost) now resolve through the
// injected `RuntimeStoresPort` instead of the `./runtime-stores.js` DB functions. NOT ONE LINE of the 5-pass-
// hardened decision logic changed — the fail-closed default, the 16-cell invariant table, the ε-clamp + ε-sweep,
// the soundness gates, computeEffort/computeDGoal/movementTowardTarget/computeFingerprint, the trip rule, and the
// SHADOW HALT+FAP compose are byte-for-byte identical to the verified admin organ. The regression self-test
// (`scripts/goal-supervisor-self-test.ts`) re-runs the verified invariant table + ε-sweep against the IN-MEMORY
// ports and proves the verdicts are unchanged on the same inputs.
//
...
//   canonical source under its OWN least-privilege identity (the MetricProbe substrate — a LATER slice's port).
...
//   tier — a SEPARATE job from the PO reconciler (2a), so the driver never grades its own progress.
//
// ── Composition (every trip-rule input reuses a now-built piece; nothing re-implemented) ──
//   dGoal   ← goal_delta_ledger ProgressSeries (RuntimeStoresPort.readProgressSeries, migration 0052)
//             RECONCILED AGAINST an external MetricProbe re-read (injected `probeInvoke`).
//   dEffort ← attempt_ledger count (RuntimeStoresPort.countAttempts) + portfolio_cost_ledger cumulative
//             (RuntimeStoresPort.readCumulativeCost), relative to the GoalContract budget_cap
//             (GoalContractStorePort → BudgetCap — the H1 envelope).
//   HALT    ← the GoalContract §4.3 LEGAL edge to HALTED (GoalContractStorePort) — DRAFTED in SHADOW.
...
//   founder-audit sink — no live goal is transitioned, no real channel is paged (draft-don't-send). The
//   enforce-flip that actually HALTs a LIVE goal is a FOUNDER ★ decision + the live goal-submission flow.
// =============================================================================

import type { BudgetCap, GoalState, RuntimeStoresPort, GoalContractStorePort } from "./ports.js";

// ── Verdict + types ──────────────────────────────────────────────────────────────────────────────────
export type GoalVerdict = "CONTINUE" | "HALT_AND_SUMMON";

/** One sample from the goal_delta_ledger (the GS's dGoal input — RuntimeStoresPort.readProgressSeries). */
...
export interface ComposeHaltOptions {
  cycle?: number | null;
  /** founder-gated: even when true this slice does NOT mutate a live goal (no live goal flow yet). */
  enforce?: boolean;
...
// =============================================================================
// EXTERNAL by construction: it RE-PROBES the acceptance metric itself (the injected `probeInvoke` under the
// GS's own least-privilege credentials), then reads the durable ledgers via the INJECTED `RuntimeStoresPort`,
// evaluates, and (on a trip) drafts the HALT + FAP. It MUTATES the goal_delta_ledger ONLY (appending its own
// independent ProgressSample — the GS's job, idempotent on (goal_id, cycle)); it transitions NO contract and
// pages NO channel (SHADOW).
...
/**
 * Re-point the GS's narrow 4-method `SupervisionStore` seam onto the injected `RuntimeStoresPort` (the 6 C12
 * durable stores). This is the WHOLE of the store-access inversion vs admin: admin bound these four to the
 * `./runtime-stores.js` DB functions (`runtimeStoresAdapter`); here they cross the injected port. The GS append
 * (`appendProgressSample`) is the GS's legitimate append-only write to goal_delta_ledger per §8.2 (distinct from
 * the reconciler, which mutates the GoalContract state machine). The three reads are the same door the
 * reconciler reads. NO DB, NO default — the consumer supplies the port (Postgres adapter / in-memory / PLOS).
 */
export function runtimeStoresPortToSupervisionStore(port: RuntimeStoresPort): SupervisionStore {
  return {
    appendProgressSample: (s) => port.appendProgressSample(s),
    readProgressSeries: (goalId) => port.readProgressSeries(goalId) as Promise<ProgressPoint[]>,
    countAttempts: (goalId) => port.countAttempts(goalId),
    readCumulativeCost: (goalId) => port.readCumulativeCost(goalId),
  };
}
 ...
 *
 * `probeInvoke` is the external re-read mechanism (the MetricProbe substrate is a LATER slice's port; binding
 * the real probe + provisioning the GS's least-privilege read-only credential is a FOUNDER-gated live-cutover
 * step). `store` is the INJECTED `SupervisionStore` (derive it from a `RuntimeStoresPort` via
 * `runtimeStoresPortToSupervisionStore`, or hand-build a fake). There is NO DB default — the residency invariant
 * forbids the package from defaulting to a concrete store. On a trip it returns a DRAFTED HaltAndFap (SHADOW) —
 * it never transitions a contract and pages no channel.
 */
...
// =============================================================================
// createGoalSupervisor — the port-COMPOSED bootstrap (the C7 organ wired onto BOTH store ports).
// =============================================================================
// The GS composes the two slice-1/slice-2 ports: the `RuntimeStoresPort` (the durable ledgers it reads/appends)
// AND the `GoalContractStorePort` (the durable contract whose §4.1 budget_cap is the H1 envelope and whose §4.3
// state is the `from` of the drafted HALT edge). `superviseGoal` SOURCES the budgetCap + contractState from the
// durable contract via the contract port (instead of the caller hand-passing them), then calls the UNCHANGED
// `runGoalSupervision`. The verified evaluator + runGoalSupervision are untouched — this factory is additive
// composition glue, not a change to the 5-pass-verified decision logic.

export interface GoalSupervisorPorts {
  /** the 6 C12 durable stores (the ledgers the GS reads/appends through). */
  store: RuntimeStoresPort;
  /** the durable PO contract — supplies the H1 budget_cap + the §4.3 lifecycle state for the goal. */
  contract?: GoalContractStorePort;
}

export interface SuperviseGoalInput {
  goalId: string;
  cycle: number;
  probeId: string;
  probeVersion: number;
  /** if omitted, sourced from the GoalContract via the contract port (the §4.1 H1 envelope). */
  budgetCap?: BudgetCap;
  /** if omitted, sourced from the GoalContract via the contract port (the §4.3 lifecycle state). */
  contractState?: GoalState;
  config?: SupervisionConfig;
}

export interface GoalSupervisor {
  /** Run one GS cycle for a goal, sourcing the budget/state envelope from the durable contract. SHADOW. */
  superviseGoal(
    input: SuperviseGoalInput,
    probeInvoke: () => Promise<{ value: number | null; note?: string }>,
    opts?: { enforce?: boolean; pagingConfigured?: boolean },
  ): Promise<RunGoalSupervisionResult>;
  /** the derived narrow store seam (exposed for diagnostics / direct runGoalSupervision use). */
  readonly supervisionStore: SupervisionStore;
}

export function createGoalSupervisor(ports: GoalSupervisorPorts): GoalSupervisor {
  const supervisionStore = runtimeStoresPortToSupervisionStore(ports.store);
  return {
    supervisionStore,
    async superviseGoal(input, probeInvoke, opts = {}) {
      let budgetCap = input.budgetCap;
      let contractState = input.contractState;
      // Compose the contract port: source the H1 envelope + lifecycle state from the durable contract when the
      // caller did not pass them. (A goal with no readable contract / no cap is handled by the verified
      // evaluator as under-instrumented — the fail-closed absolute backstops fire; nothing is waved through.)
      if ((budgetCap === undefined || contractState === undefined) && ports.contract) {
        const row = await ports.contract.readContract(input.goalId);
        if (row) {
          budgetCap = budgetCap ?? row.budgetCap;
          contractState = contractState ?? row.state;
        }
      }
      return runGoalSupervision(
        {
          goalId: input.goalId,
          cycle: input.cycle,
