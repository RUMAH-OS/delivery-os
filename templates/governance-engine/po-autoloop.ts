// =============================================================================
// The PO auto-loop / goal-lifecycle controller (C2 — the level-triggered controller that runs a goal through its
// ENTIRE §4.3 lifecycle). RS-DOS-v1 §4.3 (the state machine) · §4.4 (the level-triggered tick) · §15 (the
// reconciler is the SOLE mutator) · §37.3 / OM-INV-2/3 (the PO-organ auto-loop).
// =============================================================================
// PLATFORM EXTRACTION SLICE 4 — the port-injected mirror of `rumah-admin/src/po-autoloop-c2.ts`. The
// "ASSEMBLE THE HEART" integration: it COMPOSES the now-inverted goal-governance organs into ONE coherent loop.
// It re-implements NOTHING and — load-bearing — is NO SECOND STATE MUTATOR: this module NEVER imports or calls
// `transition()`; every contract transition goes through the reconciler's sole-mutator door
// (`applyReconcilePlan`), preserving §15. The package single-mutator source scan therefore still finds exactly
// ONE caller of `.transition()` (reconciler.ts) — this controller is not a second one.
//
// WHAT CHANGED vs admin (and ONLY this): the STORE/CONTRACT/RECONCILER access path. Admin imported the value
// functions `createContract`/`readContract` from `./goal-contract.js` (DB-coupled), `appendProgressSample`/
// `readProgressSeries` from `./runtime-stores.js` (DB-coupled), and the free `applyReconcilePlan`/`reconcile`
// from `./po-reconciler-c2.js`. THIS module imports NO DB — those four store accessors are re-pointed onto the
// injected `GoalContractStorePort` + `RuntimeStoresPort`, and `applyReconcilePlan` is the method of a
// `createReconciler(contractPort)` instance (the SOLE mutator door). `reconcile`, `c6GatedPlan`, and every organ
// it composes (`evaluateGoalSupervision`, `reviewCompletion`, `decideNextBoundary`, `summon`, the FAP adapters)
// are pure imports — BYTE-FOR-BYTE the verified admin logic. The controller is a CLOSURE over the ports
// (`createGoalLifecycleController(ports)`), so the verbatim `tick`/`runGoalLifecycle` bodies are unchanged save
// the re-pointed accessors. Residency is enforced by `residency-guard.mjs`.
//
// ── C6 GATES THE DONE BOUNDARY (the B1 fix), PRESERVED ──
//   At a §4.3 REVIEWING boundary that is NOT a GS trip, C6 (`reviewCompletion`) independently RE-PROBES the
//   metric and OWNS the verdict; `decideNextBoundary` maps it to the legal edge the controller asks the
//   reconciler to drive: COMPLETE→DONE · INCOMPLETE+reachable→PLANNING · INCOMPLETE+GS-unreachable→HALTED. A goal
//   reaches DONE IF AND ONLY IF C6's INDEPENDENT re-probe confirms COMPLETE — the loop CANNOT self-certify done
//   (the goal-level twin of the GS's loop-can't-lie property). C6 owns the DONE verdict structurally.
//
// ── SHADOW posture (unchanged) ──
//   DEFAULT = SHADOW: runGoalLifecycle runs a FIXTURE goal through the lifecycle (pure / in-memory — no DB, no
//   live goal), reports the state path + would-summons + per-tick decisions, and mutates nothing
//   (applyReconcilePlan in SHADOW drafts the would-transition with executed:false). ENFORCE proves the
//   real-transition capability against the injected store (in-memory / TEST-DB fixtures) only.
// =============================================================================
...
import { decideNextBoundary, type NextBoundary } from "./boundary-plan.js";
import {
  summon,
  defaultChannelChain,
...
// =============================================================================
// The DEFERRED Sprint Engine executor — the pluggable execution hook, DEFAULT THROWS.
// =============================================================================
// Two responsibilities, BOTH the unattended-autonomy build (panel-gated, OM-INV-2/3): (a) `admitToExecuting` —
// bring an admitted CREATED contract up to EXECUTING; and (b) `runSprint` — the unattended sprint work that
// ADVANCES the metric between ticks. Both DEFAULT to throwing "DEFERRED", exactly like §36.3's
// evaluateReachability and boundary-plan's planNextSprint, so no path silently spawns real work. The C10 sprint
// engine plugs into the `runSprint` hook via `sprint-engine.ts` `asControllerHook` (the type-enforced seam).
...
export interface SprintExecutor {
  /** ramp an admitted CREATED contract to EXECUTING via the legal §4.3 edges (C2-MIND/C9). Returns the resulting
   *  state. DEFERRED by default (throws). A real impl drives the ramp through the reconciler. */
  admitToExecuting?: (contract: GoalContractRow) => Promise<{ state: GoalState; transitions: string[]; note: string }>;
  /** the unattended work that advances the goal one increment between ticks. DEFERRED (throws). The C10 sprint
   *  engine fits here via `asControllerHook` (sprint-engine.ts). */
...
export const DEFERRED_EXECUTOR: SprintExecutor = {
  admitToExecuting: async () => {
    throw new Error(
      "po-autoloop: admitToExecuting is DEFERRED (the C2-MIND sprint planner) — bringing an admitted goal " +
        "CREATED→…→EXECUTING is the unattended-autonomy build (panel-gated); the gate/structure is built, the " +
        "ramp is sequenced (slots in with zero controller changes)",
    );
  },
  runSprint: async () => {
    throw new Error(
      "po-autoloop: runSprint is DEFERRED (the Sprint Engine) — the unattended work that spawns agents to " +
        "ADVANCE the goal between ticks is panel-gated; this slice ticks the GOVERNANCE over injected " +
        "observed-state and spawns no real work (wire the C10 engine via sprint-engine.ts asControllerHook)",
    );
  },
};
...
// executor that MOVES the metric are deferred). When the executor is wired, this becomes a real invokeProbe
// re-read; the controller is unchanged.
...
export interface LifecycleContext {
   ...
   *  governed tick. When the real executor is wired it produces these; here they are the fixture's history. */
...
// decideNextBoundary), NOT the loop's observed metric. This is a CHOICE of which legal §4.3 edge to ask the
// reconciler to drive — it is NOT a second mutator: applyReconcilePlan still performs the only transition() call,
// and the 0053 DB trigger independently re-validates the edge's legality. The plan's observed_state is set to
// C6's re-probed value, so routing and summary can never disagree at the boundary. (PURE — `reconcile` only.)
// =============================================================================
...
// goal_delta_ledger (RuntimeStoresPort) is the series — this carrier is never used there.
const shadowSeries = new Map<string, ProgressPoint[]>();

// =============================================================================
// The CONTROLLER — port-COMPOSED. createGoalLifecycleController(ports) binds the four store accessors onto the
// injected ports + the reconciler's sole-mutator door, then returns the verbatim `tick`/`runGoalLifecycle`.
// =============================================================================
export interface GovernanceControllerPorts {
  /** the durable PO contract store (supply the `createGoalContractOrgan`-wrapped store so the §4.3 TS validator
   *  + the 0053 trigger both gate every edge). The reconciler's sole-mutator door is created from THIS port. */
  contract: GoalContractStorePort;
  /** the 6 C12 durable stores (the goal-delta ledger the controller appends/reads the observed series through). */
  store: RuntimeStoresPort;
}

export interface GoalLifecycleController {
  /** ONE level-triggered governance step (idempotent). Composes observe → supervise → reconcile → (boundary)
   *  review → summon. The ONLY mutation door is the reconciler's applyReconcilePlan (§15). */
  tick(contract: GoalContractRow, ctx: LifecycleContext, tickIndex: number): Promise<TickRecord>;
  /** ADMISSION → the level-triggered TICK loop → a settled state. The full lifecycle. */
  runGoalLifecycle(submission: GoalSubmission, ctx: LifecycleContext): Promise<LifecycleResult>;
  /** the reconciler instance (the SOLE mutator door) the controller drives through — exposed for diagnostics. */
...
export function createGoalLifecycleController(ports: GovernanceControllerPorts): GoalLifecycleController {
  // The reconciler is the SOLE mutator door (§15) — created from the injected contract port. The controller
  // drives transitions ONLY through `applyReconcilePlan`; it never calls `transition()` itself.
  const reconciler = createReconciler(ports.contract);
  const applyReconcilePlan = (plan: ReconcilePlan, opts?: { enforce?: boolean }) => reconciler.applyReconcilePlan(plan, opts);
  // the four re-pointed store accessors (the WHOLE of the store-access inversion vs admin — every durable
  // read/write crosses an injected port; the verbatim bodies below are otherwise unchanged).
  const createContract = (input: CreateGoalContractInput) => ports.contract.createContract(input);
  const readContract = (goalId: string) => ports.contract.readContract(goalId);
  const appendProgressSample = (s: { goalId: string; cycle: number; value: number | null; predicted?: number | null; fixRef?: string | null }) => ports.store.appendProgressSample(s);
  const readProgressSeries = (goalId: string) => ports.store.readProgressSeries(goalId);

  async function tick(contract: GoalContractRow, ctx: LifecycleContext, tickIndex: number): Promise<TickRecord> {
    ...
    // ── (0) the DEFERRED executor — the unattended work that WOULD advance the goal between ticks. ──
    ...
    const reprobe: ExternalReprobe = {
      ...
      note: reprobeOk ? "external re-read (injected — the real invokeProbe + the executor are deferred)" : "external re-read FAILED (injected)",
    };
    ...
    //       and OWNS the verdict; decideNextBoundary maps it to the legal edge the controller asks the reconciler
    //       to drive: COMPLETE→DONE · INCOMPLETE+reachable→PLANNING · INCOMPLETE+GS-unreachable→HALTED. A goal
    //       reaches DONE IFF C6 confirms COMPLETE — the loop cannot self-certify done (B1 fix). ──
    ...
    //       the reconciler). At the C6 boundary the controller hands the reconciler the C6-GATED target (it only
    ...
    // reviewBoundary.to === "DONE": no summon (the result delivery is a later step, not a feasibility summon).
  ...
  async function runGoalLifecycle(submission: GoalSubmission, ctx: LifecycleContext): Promise<LifecycleResult> {
    const enforce = ctx.enforce === true;
    ...
    try {
      ...
      if (enforce && contract.state !== "EXECUTING") {
        return {
          ...
          contract, ramp: `admitted (CREATED) but the CREATED→EXECUTING ramp is DEFERRED — ${head(e)}. Wire admitToExecuting (the C2-MIND planner) or pre-drive the fixture to EXECUTING.`,
          ticks: [], statePath, summons, finalState: null,
        };
      }
    ...
    for (let i = 0; i < maxTicks; i++) {
      ...
      // DEFERRED C2-MIND planner (boundary-plan.planNextSprint). The lifecycle PAUSES at PLANNING awaiting that
      // re-plan; it must NOT let the reconciler re-route PLANNING→EXECUTING on the loop's (already-overridden)
      // self-claimed metric — that would re-open the B1 self-certify hole at PLANNING.
  ...
  return { tick, runGoalLifecycle, reconciler };
}

// =============================================================================
// Fixture / create-input helpers (SHADOW uses an in-memory row; ENFORCE creates a durable contract). PURE.
// =============================================================================
