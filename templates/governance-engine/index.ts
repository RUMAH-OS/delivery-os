...
// ════════════════════════════════════════════════════════════════════════════════════════════════════════════
// P2.1 — the two Execution-Infrastructure CONTRACTS (Core-owned; the node/monitoring adapters implement/consume
//   them OUTSIDE Core). Host-agnostic by construction — opaque `resource_class`/`labels`/`capabilities` strings,
//   no infra import (WAVE1 Attack-4 correction). The actual `runSprint` rewire + the Neo adapter are LATER slices.
// ════════════════════════════════════════════════════════════════════════════════════════════════════════════

// ── ExecutionProviderPort (compute-IN) — Core DEFINES + CALLS; a node adapter (infrastructure/) IMPLEMENTS it ──
export type {
  ExecutionKind,
  TrustDomain,
  PlacementConstraint,
