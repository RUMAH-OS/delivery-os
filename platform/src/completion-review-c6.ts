// =============================================================================
// The Completion Review (C6) — the DONE-VERDICT OWNER (RS-DOS-v1 §6 C6 / §9.2 / §15 / §4.3). Sprint 3.4.
// =============================================================================
// A goal does NOT self-declare done. §15: "sprint completion verdict — OWNER: C6 independent review;
// NEVER the PO." This module is that independent review. The PO reconciler (C2-LOOP) currently uses an
// "acceptance-met signal stands in for review:complete" PLACEHOLDER (src/po-reconciler-c2.ts §3); THIS
// slice provides the REAL C6 verdict the reconciler consumes to drive REVIEWING→DONE.
//
// ── Why this exists (the exact failure it prevents) ──
//   The reconciler's `acceptanceMet` reads the GS's external re-probe value to ROUTE toward the review; it
//   says (its own comment) "DONE is owned by C6/C16 — this only ROUTES". C6 closes that loop: it does NOT
//   trust the loop's claim, NOT the worker's self-report, NOT even the value the reconciler routed on. It
//   RE-PROBES the acceptance metric a SECOND time, INDEPENDENTLY, under its own least-privilege identity
//   (the MetricProbe substrate, src/metric-probe.ts), and only a sound, finite, independently-confirmed
//   metric that meets the frozen acceptance target yields COMPLETE. A false COMPLETE would mark an
//   UNFINISHED goal DONE — the precise thing author≠verifier-at-the-goal-level exists to prevent.
//
// ── FAIL-CLOSED by construction (author≠verifier at the goal level) ──
...
//   (that value MEETS the frozen target under the strict, finite-guarded comparator). EVERY other state —
//   acceptance not met, an unreadable/failed re-probe, a non-finite/null re-read, OR a malformed acceptance
//   — yields INCOMPLETE. "Cannot independently confirm done" is itself INCOMPLETE, never a silent COMPLETE.
//
// ── Composition (nothing re-implemented) ──
//   re-probe   ← metric-probe.invokeProbe (src/metric-probe.ts) — least-privilege, read-only, version-pinned,
//                under the review's OWN identity (§6 C6: "re-probing the metric under its own identity").
//   acceptance ← the GoalContract acceptance target {op,target,direction} (C2-MIND-derived, as the GS/C9/
//                reconciler consume — src/goal-supervisor-c7.ts AcceptanceShape).
//   comparison ← `acceptanceMet` (src/po-reconciler-c2.ts) — the SAME strict, finite-guarded value-at-target
//                predicate the reconciler routes on, so the routing check and the DONE verdict can never
//                disagree at the boundary (one source of truth for "is the value at/over the target").
//
// ── SHADOW posture (this slice) ──
//   reviewCompletion is a pure adjudication: it MUTATES NOTHING and transitions NO contract. It RETURNS a
//   verdict. The reconciler (the sole mutator, §15) is what would consume `complete` to drive REVIEWING→DONE
//   — and that enforce-flip on a LIVE goal is a FOUNDER ★ decision + the live goal flow (Sprint 5.3). There
//   is no live goal flow yet, so nothing live is reviewed-to-DONE here. OUT OF SCOPE: the §11-lens review;
//   the stakes-gating policy (cheap-default verify-gate vs the expensive review); driving a live goal.
// =============================================================================

import type { GoalContractRow } from "./goal-contract.js";
import type { AcceptanceShape } from "./goal-supervisor-c7.js";
import { acceptanceMet } from "./po-reconciler-c2.js"; // REUSED strict, finite-guarded value-at-target predicate
...
export interface CompletionReviewContext {
  ...
  /** Least-privilege, read-only credential resolver for the default invokeProbe binding (§36.2). */
