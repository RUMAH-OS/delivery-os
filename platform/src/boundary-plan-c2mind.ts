// =============================================================================
// Boundary next-sprint planning (C2-MIND structure) — RS-DOS-v1 §4 C2-MIND / §4.3 / §9.2 / §21.1. Sprint 3.4.
// =============================================================================
// C2-MIND is the PO's EPHEMERAL boundary-intelligence (§4): a bounded session invoked ONLY to (a) run a
// completion review (src/completion-review-c6.ts) and (b) PLAN the next sprint. This module is the (b) half:
// the DETERMINISTIC plan skeleton computed at a goal/sprint boundary, with the actual goal-decomposition into
// the next slice DEFERRED — that is LLM judgment (§15: "how to decompose into sprints/work — OWNER: C2-MIND
// boundary intelligence; NEVER a per-tick LLM"), kept behind a pluggable hook exactly like the §36.3
// reachability evaluator: the GATE/STRUCTURE is built; the JUDGMENT is deferred.
//
// ── What is DETERMINISTIC here (built) vs DEFERRED (the LLM hook) ──
//   BUILT (deterministic skeleton, no LLM):
//     * the §4.3 LEGAL next boundary given the C6 review verdict (complete → DONE · incomplete-but-reachable →
//       PLANNING(next sprint) · unreachable → HALTED) — `decideNextBoundary`, the analogue of §36.3's
//       deterministic `decideReachability` over a (possibly absent) verdict.
//     * the REMAINING-WORK summary: the desired-vs-observed gap (the SAME diff the reconciler computes,
//       §4.4 step 2) — what is left between the frozen target and the verified observed value.
//   DEFERRED (the pluggable LLM hook, throws by design):
//     * `planNextSprint` — the actual decomposition of the remaining work into the NEXT sprint's objective +
//       frozen acceptance criteria + work packages (§21.1 sizing/prioritization). This is irreducibly LLM
//       judgment; it is NOT faked. A later slice slots in the real planner with ZERO structure changes.
//
...
//   OUT OF SCOPE: the LLM next-sprint planner (deferred — hook only); driving a live goal; the Sprint Engine.
// =============================================================================

import type { GoalContractRow, GoalState } from "./goal-contract.js";
import type { AcceptanceShape } from "./goal-supervisor-c7.js";
import type { CompletionVerdict, CompletionReview } from "./completion-review-c6.js";
...
export async function planNextSprint(_input: NextSprintInput): Promise<NextSprintPlan> {
  throw new Error(
    "boundary-plan-c2mind: the next-sprint LLM decomposition is DEFERRED (Sprint 3.4 hook) — the actual " +
      "goal→slice planning (objective + frozen acceptance criteria + work packages) is C2-MIND LLM judgment, " +
      "slotted in by a later slice; the gate/structure is built, the judgment is deferred",
  );
}
...
const CONSUMES = [
  "verdict   ← C6 completion review (src/completion-review-c6.ts) — the DONE-verdict owner",
